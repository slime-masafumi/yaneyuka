...
check('外壁のリフォーム → カバー工法', top('外壁のリフォームにおすすめは？')[0], '金属サイディング');
check('防火地域のサッシ', top('防火地域のサッシの選び方').slice(0, 2), ['樹脂サッシ', '木製サッシ']);
check('フローリングの種類', top('フローリングの種類を教えて')[0], 'フローリング');
check('ベランダの雨漏り → 防水', top('ベランダの雨漏り').some((n) => /防水/.test(n)), true);
check('商品名から一般名（アスロック → ECP）', top('アスロックの特徴')[0], 'ECP');
check('シーリングの打ち替え', searchKnowledge(K, 'シーリングの打ち替え時期', 1)[0].heading.includes('打ち替え'), true);
