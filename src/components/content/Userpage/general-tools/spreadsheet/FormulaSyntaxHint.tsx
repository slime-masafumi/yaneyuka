...
const functionSignatures: Record<string, string> = {
  ...
  必要数: '必要数(数量, 1単位あたり, ロス率%)  →切り上げた必要数  例: 必要数(100, 1.6562, 5)=64',
  定尺面積: '定尺面積(幅mm, 高さmm)  →1枚あたりの面積㎡  例: 定尺面積(910,1820)=1.6562',
  尺: '尺(mm)  →尺  例: 尺(1818)=6　英字: SHAKU',
  尺ミリ: '尺ミリ(尺)  →mm  例: 尺ミリ(3)=909.09',
  ボード枚数: 'ボード枚数(面積㎡, [規格="3x6"], [ロス率%=5])  →枚  規格: "3x6" "3x8" "3x9" "3x10" "4x8"　英字: BOARD',
  本数: '本数(長さm, 定尺m, [ロス率%=0])  →本  例: 本数(38.5, 4, 5)=11　英字: ROLL',
  TSUBO: 'TSUBO(面積㎡)  →坪（坪 と同じ）',
  SHAKU: 'SHAKU(mm)  →尺（尺 と同じ）',
  BOARD: 'BOARD(面積㎡, [規格="3x6"], [ロス率%=5])  →枚（ボード枚数 と同じ）',
  ROLL: 'ROLL(長さm, 定尺m, [ロス率%=0])  →本（本数 と同じ）',
};
