...
import { dueStatus, ganttBar, ganttRange, type TaskLike } from '@/lib/teamTaskView';
...
export default function TeamGantt({
  ...
  const range = ganttRange(withDue, today);
  ...
  return (
    <div className="overflow-x-auto border border-[#3b3b3b] bg-white">
                ...
                return (
                  <tr key={t.id} className="border-t border-gray-100">
                    <td className="sticky left-0 z-10 bg-white px-2 py-1 border-r">
                      ...
                      <div className="text-[9px] text-gray-400 truncate">{[t.role, t.assigneeLabel].filter(Boolean).join('・')}</div>
                    </td>
      ...
      <p className="px-2 py-1 text-[10px] text-gray-500">
        ■ 開始〜期限　◆ 期限だけ（開始日なし）　<span className="text-red-600">赤</span> 期限切れ　<span className="text-amber-600">橙</span> 3日以内　灰 完了
      </p>
    </div>
  );
}
